...
int main(){
    ...
    matrix[4] = vector<char>({'l', 'l', 'l', 'l', 'l', 'e', 'l', '_'});
    matrix[5] = vector<char>({'g', 'g', 'g', 'g', '_', 'g', '_', 'g'});
    matrix[6] = vector<char>({'g', 'g', 'g', 'g', 'l', 'g', '_', 'g'});
    matrix[7] = vector<char>({'l', 'l', 'l', 'l', 'l', '_', 'l', 'e'});

    cout<< "Operators precedence table: " <<endl;
    for(int i =0; i < operators.size(); i++){
        for(int j =0; j < operators.size(); j++){
            cout<<matrix[i][j]<<" ";
        }
    ...
    cout<<"Enter the expr: "<<endl;
    cin>>inputExper;

    int i = 0;
    while(i < inputExper.size()){
        char currentChar = inputExper[i];
        int currentIndex = getIndex(currentChar, operators);

        if(currentChar == 'i'){
            cout<< "Identifier found: " << currentChar <<endl;
            stk.push(currentChar);
            i++;
            continue;
        }
        ...
        if(precedence == 'l' || precedence == 'e'){
            ...
            cout << "Popping element: "<< currentChar <<endl;
            stk.pop();
        }
        else{
            cout<<"Invalid operation or precedence"<<endl;
    ...
    while(stk.top() != '$'){
        char topOperator = stk.top();
        cout<<"Popping and evaluating: "<<topOperator <<endl;
        stk.pop();
    }
    return 0;
}